...
export function PostToKasModal({ usageId, totalValue, onClose, onSuccess }: PostToKasModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await inventoryService.postToKas(usageId, {
      ...
      <div className="bg-background w-full max-w-md rounded-xl shadow-lg border border-border flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Konversi pemakaian menjadi pengeluaran kas.</p>
          </div>
                ...
                >
                  ...
                  {accounts.map(acc => (
                    <option key={acc.id} value={acc.id}>
                      {acc.name} {acc.code ? `(${acc.code})` : ''} - Saldo: Rp {(acc.balance || 0).toLocaleString('id-ID')}
                    </option>
                  ))}
                </select>
              )}
        ...
        <div className="p-4 border-t border-border flex justify-end gap-3 bg-muted/50 rounded-b-xl">
          ...
          <button 
            type="submit" 
            form="post-kas-form"
            className="px-4 py-2 text-sm font-medium bg-amber-500 text-white hover:bg-amber-600 rounded-md flex items-center gap-2"
