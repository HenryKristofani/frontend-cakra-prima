...
export function AddCategoryForm({ projectId, onRefresh, parentId = null, level = 0 }: AddCategoryFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await rabService.createCategory(projectId, {
  ...
  if (!isAdding) {
    return (
      <tr>
        <td colSpan={10} className="px-4 py-2 border-x border-border/50 text-center bg-muted/10">
          <button
